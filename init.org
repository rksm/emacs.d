...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar my-packages
    '(dash dash-functional s
           ...
           elisp-slime-nav
           paredit
           clojure-mode cider clojure-cheatsheet elein
           erc znc ;; bouncer
           haskell-mode
           ghci-completion
           flycheck-haskell
...
#+BEGIN_SRC elisp
  ...
  (add-to-list 'load-path (concat (expand-file-name "~/.emacs.d/site-lisp/") "org-mode/lisp"))
  (add-to-list 'load-path (concat (expand-file-name "~/.emacs.d/site-lisp/") "org-mode/contrib/lisp"))
#+END_SRC
** emacs API
#+BEGIN_SRC emacs-lisp
  (require 'dash)
...
#+BEGIN_SRC elisp
  ...
  (defun rk/read-file-as-string (file-path)
    "read file identified by `file-path' and return its contents as
  string. If file does not exist return nil"
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-directory (concat (getenv "HOME") "/Dropbox/robertkrahn.org/notes"))
...
#+BEGIN_SRC sh :tangle no
  ...
  [ -d org-mode ] && rm -rf org-mode;
  
  # wget http://orgmode.org/org-latest.tar.gz;
  wget http://orgmode.org/cgit.cgi/org-mode.git/snapshot/release_8.2.7c.tar.gz;
  tar -xvf org-latest.tar.gz;
  rm org-latest.tar.gz;
  
  cd org-mode;
  
  sed -E 's/EMACS[[:space:]]+= emacs/EMACS = \/Applications\/Emacs.app\/Contents\/MacOS\/Emacs-10.7/' local.mk > local.mk.replace;
   mv local.mk.replace local.mk
  
  sed -E 's/prefix[[:space:]]+=.+/prefix = \/usr\/local\/share/' local.mk > local.mk.replace;
   mv local.mk.replace local.mk
    ...
    generate a bunch of `setenv' statements from it"
      `(progn
         ,@(let* ((base-cmd (if *is-a-mac*
                  ...
                  (vars-and-vals (-filter (lambda (pair) (and pair (car pair) (cadr pair)))
                                            (mapcar
                                             (lambda (line) (rk/string-match line "\\([^=]+\\)=\\(.*\\)" '(1 2)))
                                             lines))))
             (mapcar (lambda (var-value) `(setenv ,@var-value)) vars-and-vals))))
    
    (defun rk/update-shell-env ()
      "updates the shell environment (env, exec-path, ...) that is
      known to emacs from extracting relevant state from a login
      shell"
      (interactive)
    (when window-system
      (if *is-a-mac*
       (rk/set-env-from-system-shell
        ...
        "JAVA_HOME" "JAVA_TOOLS_JAR" "CLOJURESCRIPT_HOME" "DOCKER_HOST")
...
#+BEGIN_SRC elisp
  ...
  ;;; FIXME clojure-tests sneaks in...?
  (when (-contains? nrepl-connected-hook 'clojure-test-load-reporting)
    (remove-hook 'nrepl-connected-hook 'clojure-test-load-reporting))
  ...
  (define-key rk/cider-key-map (kbd "C-c M-b") 'nrepl-connection-browser)
  ;;; help
  ...
  (define-key rk/cider-key-map (kbd "s-b") 'cider-browse-ns)
  (define-key rk/cider-key-map (kbd "s-B") 'cider-browse-ns-all)

#+END_SRC

***** babel

Thanks to [[http://nakkaya.com/2013/04/06/using-clojure-with-org-babel-and-nrepl/][@nakaya.com]] and
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "C-x s-J") 'ace-swap-window)
...
#+BEGIN_SRC elisp
  (custom-set-faces
   ...
   ;; for lispy languages, heighlight matching parens sensfully
   '(show-paren-match
