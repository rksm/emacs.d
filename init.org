...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar my-packages
    '(dash dash-functional s
           ...
           elisp-slime-nav
           paredit
           clojure-mode clojure-test-mode cider clojure-cheatsheet
           ;; erc
           ;; znc ;; bouncer
           ;; haskellme
           haskell-mode
...
#+BEGIN_SRC emacs-lisp
  ...
  generate a bunch of `setenv' statements from it"
    `(progn
       ,@(let* ((cmd (format "$SHELL --login -i -c '[[ -f ~/.bashrc ]] && source ~/.bashrc; env | egrep \"^(%s)=\"'"
  ...
  (defun rk/helm-buffers-and-files ()
    ...
    (let* ((helm-ff-transformer-show-only-basename nil)
           ...
           (sources (if (projectile-project-p)
                        (-concat helm-projectile-sources-list
